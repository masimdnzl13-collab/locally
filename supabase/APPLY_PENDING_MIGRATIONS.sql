-- =============================================================================
-- APPLY_PENDING_MIGRATIONS.sql  (v4 — tamamen kendi kendine kontrol eden)
--
-- v3'TEKİ HATA: "yalnızca 7 migration (15-21) eksik, 1-14 zaten uygulanmış"
-- varsayımıyla yazılmıştı. Gerçek veritabanı taraması bunun yanlış
-- olduğunu gösterdi: admin_emails vardı ama is_demo kolonu, demo
-- fonksiyonları ve is_test_fixture kolonu yoktu — yani durum toptan değil,
-- KARIŞIKTI. "Şu tablo varsa hepsi uygulanmıştır" gibi hiçbir toptan
-- varsayım güvenilir değil.
--
-- BU SÜRÜMDE NE DEĞİŞTİ: kapsam TÜM 23 migration dosyasına genişletildi
-- (1'den 22'ye, init_schema'dan schema_migrations_log'a kadar) ve HER
-- TEK STATEMENT kendi başına güvenli hale getirildi:
--   - CREATE TABLE / CREATE INDEX          → hepsi "if not exists"
--   - CREATE POLICY (RLS)                  → önce "drop policy if exists",
--     sonra oluştur (Postgres'te CREATE POLICY IF NOT EXISTS yok, bu
--     standart eşdeğeri — sonuç aynı, hata vermez)
--   - CREATE TRIGGER                       → önce "drop trigger if exists",
--     sonra oluştur (CREATE TRIGGER IF NOT EXISTS de yok)
--   - CREATE TYPE ... AS ENUM              → pg_type kontrolüyle sarmalı
--     DO bloğu (CREATE TYPE IF NOT EXISTS de yok)
--   - ALTER TYPE ... ADD VALUE             → "if not exists" + hemen
--     ardından bir "commit" (Postgres yeni eklenen bir enum değerinin AYNI
--     transaction içinde kullanılmasına izin vermez)
--   - ALTER TABLE ... ADD COLUMN           → hepsi "if not exists"
--   - ALTER TABLE ... ADD CONSTRAINT       → pg_constraint kontrolüyle
--     sarmalı DO bloğu (ADD CONSTRAINT IF NOT EXISTS de yok)
--   - CREATE OR REPLACE FUNCTION           → zaten doğası gereği güvenli
--   - INSERT INTO ... VALUES               → hepsi "on conflict ... do
--     nothing" (admin_emails ve platform_settings'te eksikti, eklendi)
--
-- SONUÇ: bu betik BOŞ bir veritabanında da, herhangi bir şekilde KISMEN
-- uygulanmış (senin durumun gibi — bazı tablolar var, bazı kolonlar yok)
-- bir veritabanında da, TAM uygulanmış bir veritabanında da hatasız çalışır
-- ve hiçbir satırı bozmaz/çoğaltmaz. Kaç kere çalıştırırsan çalıştır sonuç
-- aynıdır. Tek bir "şu tabloya bak, hepsi tamamsa dur" mantığı YOK.
--
-- İKİNCİ İŞ — KALICI ÇÖZÜM: betiğin 22. (ve son) bölümü, veritabanında
-- hangi migration'ın gerçekten uygulandığını tutan public.schema_migrations_log
-- adında basit bir kayıt defteri oluşturur ve TÜM 23 migration için (imza
-- nesnelerine bakarak) geriye dönük kayıt işler. Bundan sonra yazılacak her
-- yeni migration dosyasının kendi sonuna bu deftere kendini yazan bir INSERT
-- eklemesi gerekiyor — ayrıntı için o bölümdeki yorumlara bak. Böylece bir
-- daha "hangi migration uygulandı?" tahmin konusu olmaz.
--
-- ÇALIŞTIRMA: Supabase Dashboard → SQL Editor → bu dosyanın tamamını
-- yapıştır → Run. En altta bir ÖZET SORGUSU var — 22 satır göreceksin,
-- her biri "applied_at" (ne zaman kayda geçti) ve "note" (bu çalıştırmada
-- mı yoksa daha önceden mi tespit edildi) sütunlarıyla. Bir hata alırsan,
-- hatanın hemen üstündeki "KAYNAK DOSYA" yorumuna bakıp orijinal migration
-- dosyasını repoda bulabilirsin.
--
-- Projenin gerçekten SİLİNMİŞ olduğunu / sıfırdan kurduğunu biliyorsan bu
-- dosya yerine FRESH_DATABASE_SETUP.sql'i de kullanabilirsin — ikisi de
-- artık aynı 23 migration'ı içeriyor, tek fark FRESH_DATABASE_SETUP.sql'in
-- en başında "veritabanı gerçekten boş mu?" diye ekstra bir güvenlik
-- kontrolü olması. Emin değilsen bu dosyayı (APPLY_PENDING_MIGRATIONS.sql)
-- kullan — her iki durumda da güvenlidir.
--
-- İÇİNDEKİLER (kronolojik sıra, 22 dosya):
--   1)  20260711003340_init_schema.sql
--   2)  20260711010000_storage_business_images.sql
--   3)  20260711020000_founder_waitlist.sql
--   4)  20260712000000_flash_deal_reservations.sql
--   5)  20260713000000_ticket_qr_and_capacity_lock.sql
--   6)  20260714000000_package_management.sql
--   7)  20260715000000_package_image.sql
--   8)  20260716000000_event_management.sql
--   9)  20260717000000_verification.sql
--   10) 20260718000000_customer_crm.sql
--   11) 20260719000000_announcements.sql
--   12) 20260720000000_cron_jobs.sql
--   13) 20260721000000_iyzico_marketplace.sql
--   14) 20260722000000_admin_panel.sql
--   15) 20260723000000_admin_allowlist_and_ai_assistant.sql
--   16) 20260724000000_pilot_mode_enum.sql
--   17) 20260724000100_pilot_mode.sql
--   18) 20260725000000_demo_data_flags.sql
--   19) 20260725000100_demo_data_load.sql
--   20) 20260820000000_admin_role_sync.sql
--   21) 20260820000100_test_lab.sql
--   22) 20260820000200_schema_migrations_log.sql
--   23) 20260821000000_multi_role.sql
--   24) 20260821000100_legacy_seed_cleanup.sql
-- =============================================================================
...
-- ============================================================================
-- KAYNAK DOSYA: supabase/migrations/20260821000000_multi_role.sql
-- ============================================================================
...
-- ==== supabase/migrations/20260821000000_multi_role.sql SONU ====


-- ============================================================================
-- KAYNAK DOSYA: supabase/migrations/20260821000100_legacy_seed_cleanup.sql
-- ============================================================================
...
-- ==== supabase/migrations/20260821000100_legacy_seed_cleanup.sql SONU ====


-- =============================================================================
-- ÖZET SORGUSU — betiğin en altında, çalıştırdıktan sonra bunu görürsün.
-- 24 satır olmalı, hepsinde bir "applied_at" tarihi olmalı. Eksik bir satır
-- varsa (24'ten az satır dönerse), o migration'ın imza nesnesi hâlâ
