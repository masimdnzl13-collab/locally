...
--
-- KULLANIM ALANI: Supabase projen HÂLÂ VAR (yalnızca duraklatılmıştı ve
-- "Restore" ile geri açtın, ya da hiç kesintiye uğramadı). Bu durumda daha
-- önceki 18 migration zaten uygulanmış durumdadır — burada yalnızca bu
-- oturumda eklenen 2 YENİ migration birleştirildi.
--
-- Projenin SİLİNMİŞ olduğunu / sıfırdan kurduğunu biliyorsan bu dosyayı
-- DEĞİL, FRESH_DATABASE_SETUP.sql dosyasını kullan.
--
-- ÇALIŞTIRMA: Supabase Dashboard → SQL Editor → bu dosyanın tamamını
-- yapıştır → Run. Tek seferlik bir işlemdir ama tekrar çalıştırılması da
-- güvenlidir (her iki kaynak migration da ON CONFLICT DO NOTHING / CREATE
-- OR REPLACE / IF NOT EXISTS kullanır, veri bozulmaz).
--
-- İÇİNDEKİLER:
--   1) 20260820000000_admin_role_sync.sql — admin_emails'e sonradan
--      eklenmiş ama zaten kayıtlı hesapları admin'e yükseltir.
--   2) 20260820000100_test_lab.sql — /test-lab-k7m2p9x4 sayfası için sabit
--      test hesapları, sahte QR senaryoları ve simülasyon fonksiyonları.
-- =============================================================================
