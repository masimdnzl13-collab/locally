  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="font-semibold text-navy-900">{refund.packageTitle}</p>
          <p className="text-xs text-muted-foreground">{refund.businessName}</p>
          <p className="text-xs text-muted-foreground">
            {refund.customerName ?? "İsimsiz"} · {refund.customerPhone ?? "—"}
          </p>
        </div>
        <span className="text-sm font-bold tabular-nums text-navy-900">{formatTL(refund.amount)}</span>
      </div>
      ...
      {mode === "idle" ? (
        <div className="mt-3 flex gap-2">
          <Button onClick={approve} disabled={isPending} variant="teal" size="sm">
        ...
        <div className="mt-3">
          <textarea
            ...
            className="w-full rounded-input border border-border bg-card px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          />
