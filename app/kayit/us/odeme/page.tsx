...
// P6 — 2/3: ödeme. Kart / PayPal hesabı bilgisi PayPal'ın onay sayfasında
// girilir; PAYPAL_CLIENT_ID/SECRET yoksa test modunda simüle edilir.
export default async function UsPaymentPage({ searchParams }: { searchParams: { canceled?: string } }) {
  ...
  const testMode = !isPayPalConfigured();
  ...
  return (
    <AuthShell title="Almost there" description={`Start your Locally subscription for ${current.signup.business.name}`}>
      <div className="space-y-4">
        ...
        <p className="text-center text-xs text-muted-foreground">Payments are processed securely by PayPal. You can pay with a card or your PayPal account.</p>
      </div>
    </AuthShell>
  );
}
