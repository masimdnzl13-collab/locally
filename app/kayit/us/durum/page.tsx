...
// P6 — 3/3: ödeme dönüşü ve kurulum durumu. Hem sağlayıcının dönüş adresi hem
...
export default async function UsStatusPage({
  ...
  // returned=1: sağlayıcının onay sayfasından dönüldü (PayPal ayrıca
  // subscription_id/token ekler; kanıt olarak kullanılmaz, webhook beklenir).
  searchParams: { returned?: string; simulated?: string };
}) {
  ...
  if (signup.status === "awaiting_payment") {
    ...
    if (!provider.isConfigured()) {
      ...
      // Gerçek ödeme: tek kanıt, sağlayıcı webhook'unun yazdığı aktif abonelik.
      const subscriptionRef = await findActiveSubscriptionRef(signup.business_id);
      if (!subscriptionRef) {
        if (searchParams.returned !== "1") redirect("/kayit/us/odeme");
        return (
      ...
      await markUsSignupPaid(signup.business_id, { mode: provider.provider === "stripe" ? "stripe" : "paypal", ref: subscriptionRef });
    }
  }
